    ...
    MembershipService<GR, TR, MR>
{
    ...
    /// known defect RG-01: the tenant-compatibility check-then-insert
    /// previously ran on a bare connection, so two concurrent "first
    /// membership" adds for the same resource in different tenants could
    /// both read an empty existing-tenants set and both insert. This is the
    /// textbook write-skew anomaly `SERIALIZABLE`/SSI is designed to catch:
    /// the two transactions' predicate reads over
    /// `get_existing_membership_tenant_ids` now conflict with each other's
    /// insert, so `PostgreSQL` aborts one with `40001` and
    /// `transaction_with_retry` retries it — the retried attempt then sees
    /// the other's committed membership and correctly returns
    /// `TenantIncompatibility`. A materialized ownership/guard row keyed on
    /// `(gts_type_id, resource_id)` would be a stronger, opt-in hardening
    /// (see `docs/db-behavior-audit.md`); SSI + retry is sufficient
    /// for correctness and is the same pattern `group_service.rs` uses
    /// throughout.
    ...
    ///
    /// Runs inside a `SERIALIZABLE` transaction with bounded retry (fixes
    /// known defect RG-14: the exists-check-then-delete previously ran on a
    /// bare connection, the same untransacted check-then-act shape as
    /// RG-01). Matches `add_membership`'s pattern for consistency, even
    /// though a plain delete-by-key has a narrower race window than an
    /// insert driven by a multi-row predicate read.
