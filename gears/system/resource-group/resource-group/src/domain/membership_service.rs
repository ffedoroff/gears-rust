    ...
    MembershipService<GR, TR, MR>
{
    ...
    /// `TenantIncompatibility`. A materialized ownership/guard row (see
    /// `docs/analysis/transaction-isolation-analysis.md`) would be a
    /// stronger, opt-in hardening; SSI + retry is sufficient for
    /// correctness and is the same pattern `group_service.rs` uses
    /// throughout.
