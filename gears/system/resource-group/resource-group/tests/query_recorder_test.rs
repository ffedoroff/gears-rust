...
//!
//! Pure-logic coverage of the recorder itself (SQL normalization, statement
//! classification, table extraction, and the aggregation methods `stats`,
//! `writes_outside_tx`, `redundant_reads_after_write`, `dump`) lives in the
//! inline `#[cfg(test)] mod tests` at the bottom of
//! `libs/toolkit-db/src/test_support.rs` — those helpers are private to that
//! module. This file only proves the wiring: attaching the recorder via
...
async fn recorder_observes_statements_and_tags_tx_membership() {
    ...
    // A write wrapped in `db.transaction_ref_mapped_with_config(...)`:
    // create_type's INSERT statements must be tagged in_tx = true.
