...
Notes:
  ...
  - The `gts.cf.core.rg.type.v1~` row above governs the JWT-authenticated REST surface only (`/api/types-registry/v1/types`, handled by `ConcreteTypeService`'s gated `create_type`/`get_type`/`list_types`/`update_type`/`delete_type`). The in-process `ClientHub` path — `dyn ResourceGroupClient`'s type-lifecycle methods, implemented by `RgService` — **bypasses `PolicyEnforcer`** for these same five operations and calls `TypeService`'s `*_unscoped` variants directly, mirroring the `ResourceGroupReadHierarchy` bypass above. Reason: account-management registers its GTS user-group types at gear init under a system actor whose `SecurityContext` carries `subject_tenant_id = Uuid::nil()` (no tenant exists yet at that point in startup); `static-authz-plugin` denies nil-tenant requests unconditionally, so routing this in-process call through the gate would make AM's own initialization fail closed in the monolith/dev stack. `RgService` audits every mutating call (create/update/delete) it takes this path for via a `tracing` event carrying the caller's `subject_id`/`subject_type`, so the bypass remains visible in logs even though it isn't policy-checked. This exception does not extend to a future microservice split where AM calls RG over HTTP instead of in-process `ClientHub`: that path would hit the gated REST surface and would need a real deployed policy for `am.system`.
