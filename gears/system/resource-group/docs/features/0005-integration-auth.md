...
5. [x] - `p1` - **IF** tenant-incompatible → **RETURN** TenantIncompatibility with a tenant-anonymous message — never the conflicting tenant ids, which are collected outside the caller's scope (; see `0004-membership.md`, tenant-compatibility algorithm) - `inst-tenant-enforce-5`
