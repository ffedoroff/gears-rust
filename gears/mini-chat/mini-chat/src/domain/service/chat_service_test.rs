...
async fn list_chats_pagination_forward_cursor() {
    ...
    let cursor = toolkit_odata::CursorV1::decode(page1.page_info.next_cursor.as_ref().unwrap())
    ...
    let cursor = toolkit_odata::CursorV1::decode(page2.page_info.next_cursor.as_ref().unwrap())
        .expect("decode cursor failed");
...
async fn list_chats_pagination_backward_cursor() {
    ...
    let prev = toolkit_odata::CursorV1::decode(page2.page_info.prev_cursor.as_ref().unwrap())
...
async fn list_chats_filter_contains_title() {
    ...
    let filter = toolkit_odata::ast::Expr::Function(
        "contains".to_owned(),
        vec![
            toolkit_odata::ast::Expr::Identifier("title".to_owned()),
            toolkit_odata::ast::Expr::Value(toolkit_odata::ast::Value::String("Report".to_owned())),
        ],
    );
...
async fn list_chats_filter_contains_title_no_match() {
    ...
    let filter = toolkit_odata::ast::Expr::Function(
        ...
        vec![
            toolkit_odata::ast::Expr::Identifier("title".to_owned()),
            toolkit_odata::ast::Expr::Value(toolkit_odata::ast::Value::String(
